...
In Rechain, Playwright is used to provide a way to write and run end-to-end tests for the web and marketing applications.
...
- To run the tests, you use turborepo and the `turbo test:e2e` command. Filtering to a single application is done using the `--filter` flag followed by the application name such as `turbo test:e2e --filter=@rechain/web`.
- To provide more flags to playwright while running it, you can do so by adding `-- --` followed by the flags. For example, `turbo test:e2e --filter=@rechain/web -- --headed` will run the tests in the web application in headed mode.
- There is also a playwright specific eslint configuration in `packages/config/eslint/playwright.js` that is used to lint the tests and is added onto the eslint configuration of each app that uses playwright using `rechain/playwright` in the extends array.
