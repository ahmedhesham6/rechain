...
```bash
bun turbo dev --filter=@rechain/web... --filter=!./packages/config/* --filter=!@rechain/ui --only
```
...
{
  "scripts": {
    "dev:web:api": "bun turbo dev --filter=@rechain/web... --filter=!./packages/config/* --filter=!@rechain/ui --only"
  }
}
```
