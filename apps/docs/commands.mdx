...
<Note>
  Rechain uses Turborepo to run commands across apps & packages in the monorepo,
...
You can start up for example only the web application by using `bun turbo dev --filter=@rechain/web`.
...
You can build for example only the web application by using `bun turbo build --filter=@rechain/web`.
...
Rechain supports all of [DrizzleKit's commands](https://orm.drizzle.team/kit-docs/commands) which are all configured for you in the `packages/db/package.json` file.

You can invoke these commands using `bun`, for example to check for collisions and inconsistencies between your database and the schema, you can run `bun --filter @rechain/db db:check`.
